describe FetchUserTweets do
  describe '#call' do
    ...
    context 'fetched some tweets' do
      let(:tweets) { 6.downto(4).map { |n| instance_double(Twitter::Tweet, n.to_s, id: n) } }
      before do
      ...
      it 'makes an API request with correct parameter' do
        subject
        expect(twitter_rest_client).to have_received(:user_timeline).with(
          hash_including(
            since_id: min_id - 1,
